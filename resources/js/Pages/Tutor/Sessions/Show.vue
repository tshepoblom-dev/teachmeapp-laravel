<template>
    <!--
        Tutor live-session page.
        Layout: full-viewport video with overlaid controls (Teams / Zoom style).
    -->
    <TutorLayout title="Live Session" :no-padding="true">
        ...
        <div class="relative w-full bg-black overflow-hidden"
            ...
            <div v-if="liveStatus !== 'ended' && (!agoraJoined || !remoteJoined)"
                ...
                <template v-else>
                    ...
                    <p class="text-white/60 text-sm">
                        <template v-if="!agoraJoined">Connecting to session…</template>
                        <template v-else>Waiting for student to join…</template>
                    </p>
                </template>
            </div>
            ...
            <div v-if="liveStatus === 'ended'"
                ...
                <p class="text-gray-300 text-sm">Earnings will be credited to your wallet shortly.</p>
                <button @click="goToBooking"
                        class="mt-3 px-6 py-2.5 bg-white text-gray-800 rounded-xl font-semibold
            ...
            <div class="absolute top-0 inset-x-0 z-20 flex items-center justify-between
                ...
                <div>
                    ...
                    <p class="text-xs text-white/60">
                        with {{ session.booking.student_name }} · {{ session.booking.duration }}min
                    </p>
                </div>
                <div class="flex items-center gap-3">
                    ...
                    <span v-else
                          class="px-2 py-0.5 rounded-full bg-yellow-500/20 text-yellow-300 text-xs">
                        Waiting for student…
                    </span>
                    ...
                    <!-- ── Low-audio advisory (Agora exception 2002) ──────────────────── -->
                    <Transition
                    ...
                    >
                        <span v-if="lowAudioWarning"
                            class="flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-medium"
                            style="background:rgba(234,179,8,.2); color:#fde68a;">
                            🔉 Student audio is low
                        </span>
                    </Transition>
                </div>
            </div>
            ...
            <div v-if="liveStatus !== 'ended'"
                ...
                <!-- Centre: start / end -->
                <div class="flex items-center gap-2">
                    <!-- Start session — only when student has joined -->
                    <button v-if="liveStatus === 'active'"
                            @click="startSession" :disabled="starting"
                            class="flex flex-col items-center gap-0.5 px-5 py-2 bg-green-600 hover:bg-green-700
                                   disabled:opacity-50 rounded-xl text-white text-xs font-semibold
                                   transition-colors w-20">
                        <span class="text-lg">▶</span>
                        {{ starting ? 'Starting…' : 'Start' }}
                    </button>
                    <!-- Waiting hint -->
                    <span v-if="liveStatus === 'waiting'"
                          class="text-white/40 text-xs italic">
                        Waiting for student…
                    </span>
                    <!-- End session -->
                    <button v-if="liveStatus === 'active' || liveStatus === 'in_progress'"
                            ...
                            class="flex flex-col items-center gap-0.5 px-4 py-2 bg-red-600 hover:bg-red-700
                                   rounded-xl text-white text-xs font-semibold transition-colors w-16">
                        <span class="text-lg">⏹</span>
                        End
                    </button>
                </div>
            ...
            >
                <div v-if="chatOpen"
                     class="absolute top-0 right-0 bottom-0 z-20 w-72 sm:w-80
                            flex flex-col bg-gray-900/90 backdrop-blur-md border-l border-white/10">
                    ...
                    <div ref="chatContainer"
                        ...
                        <div v-if="chatMessages.length === 0"
                             class="text-center text-white/30 text-xs mt-8">No messages yet</div>
                        <div v-for="msg in chatMessages" :key="msg.id"
                             :class="msg.is_system_message
                                 ? 'text-center'
                                 : msg.sender_id === currentUserId ? 'flex justify-end' : 'flex justify-start'">
                            <div v-if="msg.is_system_message" class="text-[11px] text-white/40 italic px-2">
                                {{ msg.message }}
                            </div>
                            <div v-else
                                ...
                                <p class="text-[10px] font-semibold opacity-60 mb-0.5">
                                    {{ msg.sender_id === currentUserId ? 'You' : session.booking.student_name }}
                                </p>
                                {{ msg.message }}
                            </div>
                        </div>
                    ...
                    <div class="px-3 py-3 border-t border-white/10 flex gap-2">
                        ...
                        <button @click="sendChatMessage" :disabled="!chatInput.trim()"
                                class="px-3 py-2 bg-indigo-600 hover:bg-indigo-700 disabled:opacity-40
                                       text-white rounded-xl text-sm transition-colors">↑</button>
                    </div>
                </div>
            </Transition>
        </div>
        ...
        <Teleport to="body">
            <div v-if="showEndConfirm"
                ...
                <div class="bg-white rounded-2xl p-6 max-w-sm w-full shadow-2xl">
                    <h3 class="text-lg font-semibold text-gray-800 mb-1">End Session?</h3>
                    <p class="text-sm text-gray-500 mb-4">
                        Ending will complete the session and release escrow to your wallet. This cannot be undone.
                    </p>
                    <textarea v-model="endReason" placeholder="Optional reason or notes…" rows="2"
                              class="w-full border border-gray-200 rounded-lg px-3 py-2 mb-4 text-sm
                                     focus:outline-none focus:ring-2 focus:ring-red-400 resize-none" />
                    <div class="flex gap-3">
                        <button @click="showEndConfirm = false"
                ...
                <div v-if="showReportModal"
                    ...
                    <div class="bg-white rounded-2xl p-6 max-w-md w-full shadow-2xl">
                        <div class="flex items-start justify-between mb-4">
                            <div>
                                ...
                                <p class="text-sm text-gray-500 mt-0.5">
                                    Report {{ session.booking.student_name }} to our moderation team.
                                </p>
                            </div>
                        ...
                        <select v-model="reportForm.reason"
                            ...
                            <option>Disruptive behaviour during session</option>
                            <option>Other</option>
                        </select>
...
const { listenToSession, leaveSession } = useEcho()
...
const currentUserId  = props.current_user_id ?? props.session?.booking?.tutor_id
...
async function sendChatMessage() {
    ...
    if (!msg) return
    chatInput.value = ''
    chatMessages.value.push({ id: Date.now(), sender_id: currentUserId, message: msg, is_system_message: false })
    await scrollChat()
   /* router.post(route('sessions.chat.send', props.session.id), { message: msg }, {
        ...
        try {
            await axios.post(route('sessions.chat.send', props.session.id), { message: msg })
        } catch {
...
const lowAudioWarning = ref(false)  // shown in UI when remote audio is too quiet
...
function subscribeToSession() {
    const { listenToSession: listen, leaveSession: leave } = useEcho()

    const stopStatus = listen(props.session.id, 'session.status', (payload) => {
        const prev = liveStatus.value
        liveStatus.value = payload.new_status
        if (payload.new_status === 'in_progress' && prev !== 'in_progress') {
            toast.success('Session started — billing clock is running.')
            startTimer(payload.started_at)
        }
        if (payload.new_status === 'active' && prev === 'waiting') {
            toast.info('Student has joined the session.')
        }
        if (payload.new_status === 'ended') { stopTimer(); leaveAgoraChannel() }
    })

    const stopChat = listen(props.session.id, 'session.chat', (payload) => {
        chatMessages.value.push(payload.message)
        if (!chatOpen.value) unreadCount.value++
        else scrollChat()
    })

    echoCleanup = () => { stopStatus(); stopChat(); leave(props.session.id) }
}

function goToBooking() {
    router.visit(route('tutor.bookings.show', props.session.booking.id))
}
...
async function initAgora() {
    const { agora_app_id: appId }                                      = props
    const { channel_name: channelName, agora_token: token, agora_uid: uid } = props.session

    if (!appId || !channelName) { agoraError.value = 'Agora credentials not configured.'; return }

    const { hasAudio, hasVideo } = await checkDevices()
    ...
    client.on('user-unpublished', (remoteUser, mediaType) => {
        if (mediaType === 'video') remoteJoined.value = false
    })
    ...
    client.on('connection-state-change', (state) => {
        if (state === 'DISCONNECTED') agoraJoined.value = false
    })

    // ── Exception handler — replaces unformatted SDK console warnings ─────────
    // Without this, Agora emits raw DEBUG/WARNING lines for codes like 2002
    // (AUDIO_OUTPUT_LEVEL_TOO_LOW) directly via its internal logger.
    ...
    try {
        ...
        if (hasAudio) {
            ...
            } catch { agoraError.value = 'Microphone unavailable.' }
        }
        if (hasVideo) {
            try {
                cameraTrack = await AgoraRTC.createCameraVideoTrack()
                tracks.push(cameraTrack)
                cameraTrack.play('agora-local')
            } catch {
                if (!agoraError.value) agoraError.value = 'Camera unavailable — audio only.'
            }
        }
        if (tracks.length) await client.publish(tracks)
    } catch (err) {
        agoraError.value = `Could not connect: ${err.message ?? err}`
...
function startSession() {
    starting.value = true
    router.post(route('tutor.sessions.start', props.session.id), {}, {
...
function endSession() {
    ending.value = true
    router.post(route('tutor.sessions.end', props.session.id), { reason: endReason.value }, {
        onSuccess: () => { showEndConfirm.value = false; liveStatus.value = 'ended' },
        onError:   () => toast.error('Could not end session. Please try again.'),
        onFinish:  () => { ending.value = false },
    })
}
</script>
