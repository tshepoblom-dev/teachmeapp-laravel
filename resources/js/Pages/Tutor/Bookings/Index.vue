<template>
    <TutorLayout title="Bookings">
        <!-- Filter tabs -->
        <div class="flex gap-2 mb-5 flex-wrap">
            <Link
                v-for="s in ['all', ...statuses]" :key="s"
                :href="s === 'all' ? route('tutor.bookings.index') : route('tutor.bookings.index', { status: s })"
        ...
        <div class="bg-white min-w-[640px]">
            <table class="w-full text-sm">
                ...
                <tbody class="divide-y divide-gray-100">
                    ...
                    <tr v-for="b in bookings.data" :key="b.id" class="hover:bg-gray-50">
                        <td class="px-4 py-3 text-gray-800">{{ b.student?.name ?? '—' }}</td>
                        <td class="px-4 py-3 text-gray-700">{{ b.subject }}</td>
                        <td class="px-4 py-3 text-gray-600">{{ formatDate(b.scheduled_at) }}</td>
                        ...
                        <td class="px-4 py-3">
                            <Link :href="route('tutor.bookings.show', b.id)" class="text-indigo-600 hover:underline text-xs">View</Link>
                        </td>
                    </tr>
                </tbody>
            </table>
            ...
            <div v-if="bookings.last_page > 1" class="px-4 py-3 border-t border-gray-100 flex gap-2">
                <Link
                    v-for="link in bookings.links" :key="link.label"
                    :href="link.url ?? '#'"
